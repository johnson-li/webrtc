...
#tmux send-key -t ${session_name}:4 'conda activate webrtc-exp7' Enter
#tmux send-key -t ${session_name}:4 'cd /tmp/webrtc/python_src' Enter
#tmux send-key -t ${session_name}:4 'python experiment/fakewebcam.py' Enter
sleep 1
...
tmux send-key -t ${session_name}:3 'python stream.py > /tmp/webrtc/logs/stream.log 2>&1' Enter
