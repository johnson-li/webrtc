...
tmux send-key -t ${session_name}:0 'cd ~/Workspace/eatw; authbind python -m dns.server > /tmp/webrtc/logs/$(date +"%Y-%m-%d-%H-%M-%S").dns.log' Enter
tmux send-key -t ${session_name}:1 'cd ~/Workspace/eatw; python -m dns.monitor_server' Enter
