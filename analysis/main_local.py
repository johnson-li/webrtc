...
RESULT_DIR = os.path.expanduser('~/data/exp5')
...
def parse_args():
    ...
    parser.add_argument('-d', '--path', default=RESULT_DIR, help='Plot statics')
...
def main():
    ...
    path = args.path
    sequences = []
    reg: LinearRegression = parse_sync(plot=False, path=path)
    ...
    if args.recursive:
        for p in os.listdir(path):
            ...
            if not args.accuracy_only:
                frames = parse_results_latency(p, reg)
        ...
        if not args.accuracy_only:
            frames = parse_results_latency(path, reg)
