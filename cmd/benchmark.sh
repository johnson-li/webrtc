...
results_path=~/Workspace/webrtc/results/benchmark
mkdir -p $log_path
...
do
    ...
    cp $log_path/udp_server.log $results_path/sink/$bitrate
    python -m benchmark.client -s 195.148.127.108 -d $bitrate -b udp_pour -l $log_path -t 30
    mkdir -p $results_path/pour/$bitrate
    cp $log_path/udp_client.log $results_path/pour/$bitrate
    cp $log_path/udp_server.log $results_path/pour/$bitrate
done
